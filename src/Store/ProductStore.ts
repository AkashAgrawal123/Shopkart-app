...
const useProductStore: StateCreator<ProductStore> = (
  ...
  handleHeartClick: (id: number) => {
    items((state) => {
      ...
      let updatedHeartItems;
      if (existingHeartItem) {
        updatedHeartItems = state.wishlistItemsArray.filter(
          (item: Product) => item.id !== id,
        );
      } else {
        ...
        if (productToAdd) {
          updatedHeartItems = [
            ...state.wishlistItemsArray,
            { ...productToAdd, isLiked: true },
          ];
        }
      }
      ...
      return {
        ...
        wishlistItemsArray: updatedHeartItems,
      };
    });
  },
});

const persistedUseProductStore = persist(useProductStore, {
  name: "ProductStore",
